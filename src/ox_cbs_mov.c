...
void ox_cbs_mov_get(evhtp_request_t *req, void *arg)
{
  ...
  if((strlen(uri) == 4 || strlen(uri) == 5) &&
    ...
    if((fd = open(vars.root_path, O_RDONLY)) == -1) {
      ...
      evbuffer_add_printf(req->buffer_out, "<html><body><h1>mov get: no key</h1></body></html>");
    }
    else {
      if (fstat(fd, &st) < 0) {
    ...
    evhtp_send_reply(req, EVHTP_RES_OK);
    LOG_PRINT(LOG_DEBUG, "============_mov_get() DONE!===============");
...
void ox_cbs_mov_lock(evhtp_request_t *req, void *arg)
{
  ...
  // storage setting
  if (vars.mode == 1) {
    lock_mov_rst = ox_mov_lock(ox_req, req);
  }
  else {
    lock_mov_rst = ox_mov_lock_db(ox_req, req);
  }
 ...
 forbidden:
  ...
  evhtp_headers_add_header(req->headers_out, evhtp_header_new("Server", vars.server_name, 0, 1));
  evhtp_send_reply(req, EVHTP_RES_OK);
  LOG_PRINT(LOG_DEBUG, "============_mov_lock() FORBIDDEN!===============");
  goto done;
 ...
 err:
  ...
  evhtp_headers_add_header(req->headers_out, evhtp_header_new("Server", vars.server_name, 0, 1));
  evhtp_send_reply(req, EVHTP_RES_OK);
  LOG_PRINT(LOG_DEBUG, "============_mov_lock() ERROR!===============");
...
void ox_cbs_mov_unlock(evhtp_request_t *req, void *arg)
{
  ...
  int unlock_mov_rst = -1;

  // storage setting
  if (vars.mode == 1) {
    unlock_mov_rst = ox_mov_unlock(ox_req, req);
  }
  else {
    unlock_mov_rst = ox_mov_unlock_db(ox_req, req);
  }
