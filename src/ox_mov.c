...
int ox_mov_get(ox_req_mov_t *req, evhtp_request_t *request)
{
  ...
  snprintf(whole_path_lock, 512, "%s/%d/%d/%s/lock", vars.mov_path, lvl1, lvl2, req->md5);
  snprintf(whole_path_lock, 512, "%s/%d/%d/%s/lock.%s", vars.mov_path, lvl1, lvl2, req->md5, req->passwd);
  LOG_PRINT(LOG_DEBUG, "whole_path: %s", whole_path);

  if(ox_isdir(whole_path) == -1) {
    LOG_PRINT(LOG_DEBUG, "Mov %s is not existed!", req->md5);
    goto err;
  }
...
int ox_mov_get_db(ox_req_mov_t *req, evhtp_request_t *request)
{
  ...
  snprintf(cache_key, CACHE_KEY_SIZE, "MOV_%s.lock", req->md5);
  snprintf(cache_key_passwd, CACHE_KEY_SIZE, "MOV_%s.lock.%s", req->md5, req->passwd);
...
int ox_mov_del(ox_req_mov_t *req, evhtp_request_t *request)
{
  ...
  LOG_PRINT(LOG_DEBUG, "_mov_del() start processing admin request...");
  char whole_path[512];
  int lvl1 = ox_strhash(req->md5);
  int lvl2 = ox_strhash(req->md5 + 3);
  snprintf(whole_path, 512, "%s/%d/%d/%s", vars.mov_path, lvl1, lvl2, req->md5);
...
int ox_mov_del_db(ox_req_mov_t *req, evhtp_request_t *request)
{
  ...
  LOG_PRINT(LOG_DEBUG, "ox_mov_del_db() start processing admin request...");

  char cache_key[CACHE_KEY_SIZE+5];
  snprintf(cache_key, 40, "MOV_%s", req->md5);
  LOG_PRINT(LOG_DEBUG, "original key: %s", cache_key);
...
int ox_mov_lock(ox_req_lock_t *req, evhtp_request_t *request)
{
  ...
  LOG_PRINT(LOG_DEBUG, "_mov_lock() start processing admin request...");
  char whole_path[512];
  ...
  snprintf(whole_path, 512, "%s/%d/%d/%s/lock", vars.mov_path, lvl1, lvl2, req->md5);
  snprintf(whole_path_passwd, 512, "%s/%d/%d/%s/lock.%s", vars.mov_path, lvl1, lvl2, req->md5, req->passwd);
  LOG_PRINT(LOG_DEBUG, "whole_path: %s", whole_path);
...
int ox_mov_lock_db(ox_req_lock_t *req, evhtp_request_t *request)
{
  ...
  LOG_PRINT(LOG_DEBUG, "ox_mov_lock_db() start processing admin request...");
