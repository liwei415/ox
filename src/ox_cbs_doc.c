...
void _cbs_doc_post(evhtp_request_t *req)
{
  int post_size = 0;
  char *buff = NULL;
  int err_no = 0;
...
void _cbs_doc_get(evhtp_request_t *req)
{
  char *md5 = NULL;
  char *fmt = NULL;
  char *type = NULL;
...
void _cbs_doc_del(evhtp_request_t *req)
{
  char md5[35];
  int err_no = 0;
...
void ox_cbs_doc_del(evhtp_request_t *req, void *arg)
{
 ...
 err:
  ...
  evhtp_headers_add_header(req->headers_out, evhtp_header_new("Content-Type", "text/html", 0, 0));
  evhtp_send_reply(req, EVHTP_RES_NOTFOUND);
  LOG_PRINT(LOG_DEBUG, "============ox_cbs_doc_del() ERROR!===============");
