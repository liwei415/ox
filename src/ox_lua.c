...
static int ox_type(lua_State *L)
{
  lua_arg_t *larg = pthread_getspecific(thread_key);
  lua_pushstring(L, larg->trans_type);
  LOG_PRINT(LOG_DEBUG, "ox_type: %s", larg->trans_type);
  return 1;
}

static int ox_ret(lua_State *L)
{
  lua_arg_t *larg = pthread_getspecific(thread_key);
...
static int lua_log_print(lua_State *L)
{
  ...
  LOG_PRINT(log_level, "ox_lua: %s", log_str);
  return 0;
}
...
int ox_lua_convert(MagickWand *im, ox_req_img_t *req)
{
  ...
  if(req->thr_arg->L != NULL) {
    lua_arg_t *larg = (lua_arg_t *)malloc(sizeof(lua_arg_t));
    if(larg == NULL) {
      return -1;
    }
    larg->lua_ret = ret;
    ...
    luaL_dofile(req->thr_arg->L, vars.script_name);
