...
void ox_cbs_img_lock(evhtp_request_t *req, void *arg)
{
  ...
  // storage setting
  if (vars.mode == 1) {
    lock_img_rst = ox_img_lock(ox_req, req);
  }
  else {
    lock_img_rst = ox_img_lock_db(ox_req, req);
  }
...
void ox_cbs_img_unlock(evhtp_request_t *req, void *arg)
{
  ...
  int unlock_img_rst = -1;

  // storage setting
  if (vars.mode == 1) {
    unlock_img_rst = ox_img_unlock(ox_req, req);
  }
  else {
    unlock_img_rst = ox_img_unlock_db(ox_req, req);
  }
